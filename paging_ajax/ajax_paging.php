<?php
	...
	if(isset($_POST["page"]))
    {
		...
		$paging->text_sql = "select id,ten,tenkhongdau,photo,thumb,giacu,gia,masp,mota from table_product where hienthi=1 and id_list=".$_POST["id_danhmuc"]." and noibat=1 and type='sanpham' order by stt asc";
...
<?php for($i=0,$count_product=count($product);$i<$count_product;$i++){	?>
    <div class="item" itemscope itemtype="http://schema.org/Product">
            <p class="sp_img hover_sang1 zoom_hinh"><a href="<?=$product[$i]['tenkhongdau']?>-<?=$product[$i]['id']?>.html" title="<?=$product[$i]['ten']?>">
